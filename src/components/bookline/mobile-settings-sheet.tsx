...
export function MobileSettingsSheet({ children }: MobileSettingsSheetProps) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={handleOpenChange}>
            ...
            <DialogContent className="max-w-2xl w-full p-0 rounded-full overflow-hidden border-none bg-background shadow-xl fixed bottom-4 left-1/2 -translate-x-1/2 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:slide-out-to-bottom-full data-[state=open]:slide-in-from-bottom-full">
                 <DialogTitle className="sr-only">Paramètres</DialogTitle>
                 <div className="h-[65vh] bg-background overflow-hidden flex flex-col">
                    {SettingsContent}
                 </div>
            </DialogContent>
        </Dialog>
