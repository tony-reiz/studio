...
interface LiquidChronicProps {
  className?: string;
}

export function LiquidChronic({ className }: LiquidChronicProps) {
  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      void main() {
        ...
        // distortion sur toute la surface (plus de cercle)
        float t_distort = uTime * 0.4;
        vec2 noise_coord = uv * 0.2 + t_distort;

        float dx = snoise(noise_coord);
        float dy = snoise(noise_coord + vec2(10.0));

        vec2 distortion = vec2(dx, dy);

        float strength = 0.15;
        vec2 p = uv + distortion * strength;

        // chromatic aberration
        float ca = 0.01;
        vec2 offset = distortion * ca;

        float ratio = uResolution.x / uResolution.y;

        float r = compute_color((p - offset) * vec2(ratio, 1.0), t).r;
        float g = compute_color(p * vec2(ratio, 1.0), t).g;
        float b = compute_color((p + offset) * vec2(ratio, 1.0), t).b;
    ...
    // ✅ resize basé sur le container (FIX IMPORTANT)
    const resize = () => {
      if (!canvas) return;
